...
function AssemblyCanvas({
  ...
  const ask = useCallback(
    async (query: string) => {
      if (!context.length) {
        onToast('Drag at least 1 source into Chat (connect edge) or select in the panel')
        return
      }
      setBusy(true)
      ...
      try {
        ...
        onToast(e instanceof Error ? e.message : 'Chat error')
      } finally {
        setBusy(false)
      }
    },
  ...
  const onUpload = async (files: FileList | null) => {
    ...
    try {
      ...
      onToast(`Added ${res.added.length} source(s)`)
    } catch (e) {
      onToast(e instanceof Error ? e.message : 'Upload error')
    }
  }
  ...
  const runStudio = async (tool: string, label: string) => {
    ...
    if (!srcs.length) {
      onToast('Need at least 1 source')
      return
    }
    setStudioBusy(true)
    try {
      ...
      onToast(`Created ${label}`)
    } catch (e) {
      onToast(e instanceof Error ? e.message : 'Studio error')
    } finally {
      setStudioBusy(false)
    }
  }
      ...
      <div className="workspace">
          ...
          <div className="side-body">
            ...
            {sources.map((s) => (
              <div
                ...
                title="Click to add to context + canvas"
              >
                {s}
              </div>
            ))}
            {!sources.length && <p className="muted text-sm">No sources yet — upload to get started</p>}
          </div>
        </aside>
        ...
        >
          <div className="canvas-hint">
            Assembly Canvas · drag source → connect to Chat · board saved on server
          </div>
          {!ready ? (
        ...
        <aside className="side-panel right">
          <div className="side-hd">Studio · assemble artifacts</div>
          <div className="side-body">
            <p className="muted text-sm" style={{ margin: 0 }}>
              Use selected context ({context.length || 'all'} sources) to generate interactive widgets on the canvas.
            </p>
            ...
            <p className="muted text-sm">
              Scored quiz · flip flashcards · mind map expand — drag to arrange on the board.
            </p>
          </div>
        </aside>
      </div>
...
export function Workspace({
  ...
  const exportNb = async () => {
    try {
      ...
      onToast(e instanceof Error ? e.message : 'Export error')
    }
  }
