...
_VI_STOPWORDS = {
    "là", "của", "và", "có", "trong", "cho", "với", "các", "một", "những",
    "được", "này", "đó", "khi", "như", "từ", "về", "thì", "để", "hay",
    ...
    "gì", "nào", "sao", "ai", "đâu", "bao", "nhiêu", "tại", "vì", "mà",
}
...
def rewrite_queries(query: str) -> List[str]:
    ...
    if not base.endswith("?"):
        variants.append(f"{stripped} là gì")
        variants.append(f"giải thích {stripped}")
    else:
        variants.append(stripped)
        if keywords:
            variants.append(f"tài liệu nói về {' '.join(keywords[:5])}")
