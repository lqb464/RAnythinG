...
export function QuizWidget({ items }: { items: QuizItem[] }) {
  ...
  if (!items.length) return <p className="muted">Không có câu hỏi</p>
  return (
    <div>
      {items.slice(0, 8).map((item, i) => {
        const q = item.question || `Câu ${i + 1}`
        ...
        return (
          <div key={i} className="quiz-item">
            ...
            {scores[i] && (
              <div className="cite">{scores[i] === 'correct' ? 'Đúng' : `Sai · đáp án: ${correct}`}</div>
            )}
          </div>
        )
      })}
...
export function FlashcardsWidget({ items }: { items: FlashItem[] }) {
  ...
  if (!cards.length) return <p className="muted">Không có thẻ</p>
  ...
  return (
    <div>
      ...
      <div className="flash-nav">
        ...
        <span className="cite">
          {idx + 1}/{cards.length} · bấm thẻ để lật
        </span>
        <button
          type="button"
          className="btn btn-sm"
