...
def _format_answer_html(text: str) -> str:
    return re.sub(
        r"\[(\d+)\]",
        r'<sup class="cite" title="Source \1">\1</sup>',
        text,
    )


# Frontend SPA (React build) is mounted after all API routes — see bottom of module.
...
async def chat(notebook_id: str, body: ChatRequest, user: AuthUser = Depends(require_user)):
    ...
    if len(agent.chunks) == 0:
        raise HTTPException(400, "No RAG index yet — upload documents first")
...
async def add_note(notebook_id: str, body: NoteCreate, user: AuthUser = Depends(require_user)):
    ...
    if len(agent.chunks) > 0:
        await loop.run_in_executor(
            None, agent_cache.add_document, notebook_id, f"[Note] {title}", content
        )
    else:
        await loop.run_in_executor(None, agent_cache.rebuild_agent, notebook_id)
    return notes
...
async def update_note(
    ...
    if old_title:
        agent_cache.remove_document(notebook_id, f"[Note] {old_title}")
    agent = await agent_cache.async_get_agent(notebook_id)
    loop = asyncio.get_event_loop()
    if len(agent.chunks) > 0:
        await loop.run_in_executor(
...
async def delete_note(notebook_id: str, note_id: str, user: AuthUser = Depends(require_user)):
    ...
    agent_cache.remove_document(notebook_id, f"[Note] {note_to_remove['title']}")
    return notes
...
async def note_to_source(notebook_id: str, note_id: str, user: AuthUser = Depends(require_user)):
    ...
    clean_title = re.sub(r'[^a-zA-Z0-9_\-À-ỹ ]', '', note['title']).strip() or "Note"
    filename = f"Note - {clean_title[:40]}.txt"
    data = f"# {note['title']}\n\n{note['content']}".encode("utf-8")
    saved_name = store.save_upload_bytes(notebook_id, filename, data)
    if not saved_name:
        raise HTTPException(status_code=500, detail="Could not save source file")
...
async def studio_generate(
    ...
    if handler is None:
        raise HTTPException(404, f"Studio tool '{tool}' not found")
    agent = await agent_cache.async_get_agent(notebook_id)
    if len(agent.chunks) == 0:
        raise HTTPException(400, "No RAG index yet — upload documents first")
    ...
    try:
        ...
        raise HTTPException(500, f"Could not generate Studio content: {exc}") from exc
...
async def build_graph(
    ...
    agent = await agent_cache.async_get_agent(notebook_id)
    if len(agent.chunks) == 0:
        raise HTTPException(400, "No index yet — upload documents first")
    ...
    if not result.get("ok"):
        raise HTTPException(400, result.get("error", "Graph build failed"))
...
async def ask_graph_entity(
    ...
    if not (body.entity_id or "").strip():
        raise HTTPException(400, "entity_id is empty")
    agent = await agent_cache.async_get_agent(notebook_id)
    if agent.knowledge_graph is None:
        raise HTTPException(400, "No knowledge graph yet — click Build Graph")
    loop = asyncio.get_event_loop()
...
async def get_settings(user: AuthUser = Depends(require_user)):
    ...
    return {
        ...
        "language_hint": os.getenv("UI_LANGUAGE", "en"),
    }
...
async def external_create_project(
    ...
    if not project_id:
        raise HTTPException(400, "project_id cannot be empty")
...
async def external_query(
    ...
    agent = await agent_cache.async_get_agent(project_id)
    if len(agent.chunks) == 0:
        raise HTTPException(400, "This project has no documents yet")
