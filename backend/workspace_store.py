"""Filesystem storage for workspaces (local / no Postgres)."""
...
WORKSPACES_ROOT = Path(os.getenv("WORKSPACES_DIR", "./data/workspaces"))
_LEGACY_ROOT = Path("./data/notebooks")


def _root() -> Path:
    """Prefer workspaces/; fall back to legacy notebooks/ if present."""
    if WORKSPACES_ROOT.exists() or not _LEGACY_ROOT.exists():
...
def _meta_path(workspace_id: str) -> Path:
    return _root() / workspace_id / "meta.json"


def _notes_path(workspace_id: str) -> Path:
    return _root() / workspace_id / "notes.json"


def _studio_path(workspace_id: str) -> Path:
    return _root() / workspace_id / "studio_outputs.json"


def _chat_path(workspace_id: str) -> Path:
    return _root() / workspace_id / "chat_history.json"


def _sources_dir(workspace_id: str) -> Path:
    return _root() / workspace_id / "sources"


def _index_path(workspace_id: str) -> Path:
    return _root() / workspace_id / "index.faiss"


def _metadata_path(workspace_id: str) -> Path:
    return _root() / workspace_id / "index.json"
...
def create_notebook(
    ...
    workspace_id = notebook_id or str(uuid.uuid4())[:8]
    folder = _root() / workspace_id
    folder.mkdir(parents=True, exist_ok=True)
    _sources_dir(workspace_id).mkdir(exist_ok=True)
    ...
    with open(_meta_path(workspace_id), "w", encoding="utf-8") as f:
        json.dump(meta, f, ensure_ascii=False, indent=2)

    _save_json(_notes_path(workspace_id), [])
    _save_json(_chat_path(workspace_id), [])
    return meta
...
def _parsed_dir(workspace_id: str) -> Path:
    return _sources_dir(workspace_id) / "_parsed"
...
def _read_parsed_text(workspace_id: str, original_name: str) -> str:
    parsed_dir = _parsed_dir(workspace_id)
    new_path = parsed_dir / f"{original_name}.txt"
    if new_path.exists():
        return new_path.read_text(encoding="utf-8", errors="ignore")
    legacy = _sources_dir(workspace_id) / f"{original_name}.parsed.txt"
    if legacy.exists():
...
def collect_documents(notebook_id: str) -> List[Tuple[str, str]]:
    ...
    for note in load_notes(notebook_id):
        docs.append((f"[Note] {note['title']}", note["content"]))
    return docs
...
def _assembly_path(workspace_id: str) -> Path:
    return _root() / workspace_id / "assembly.json"
